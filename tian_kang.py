...
def main():
        ...
        if mode == "員工專區":
            ...
            if st.button("驗證進入"):
                ...
                if not match.empty:
                    ...
                    else: st.error("❌ 無權限帳號"); st.stop()
                    st.rerun()
                else: st.error("❌ 管理身分驗證失敗")
            ...
            with st.form("reg_form"):
                ...
                if st.form_submit_button("確認註冊"):
                    ...
                    is_valid = not df_emp[(df_emp['姓名']==reg_name) & (df_emp['身分證']==reg_id)].empty
                    if is_admin or is_valid:
    ...
    if role == 5:
        ...
        if role == 4: # 會計專屬
            t_acct = st.tabs(["🏥 勞健保紀錄管理", "👤 員工名單"])
            with t_acct[0]:
                st.subheader("🏥 勞健保歷史紀錄維護")
            ...
            with tabs[0]: 
                if role == 1:
                    with st.sidebar.expander("➕ [管理] 建立新月份"):
                        ...
                        if st.button("執行建立"):
                            ...
                            if not df_pay.empty:
                                # 抓取每位員工最新的備註紀錄
                                latest_remarks = df_pay.sort_values(by=['姓名', '月份'], ascending=[True, False]).drop_duplicates('姓名', keep='first')[['姓名', '備註']]
                                df_temp = df_emp.merge(latest_remarks, on='姓名', how='left')
                                df_temp['備註'] = df_temp['備註'].fillna("") # 若無紀錄則為空
                            else:
                                df_temp = df_emp.copy()
                                df_temp['備註'] = ""
                            ...
                            new_r = pd.DataFrame({
                                ...
                                "店別": df_emp["店別"],
                                "姓名": df_emp["姓名"],
                                "職務加給": 0, "加班津貼": 0, "店毛利成長獎金": 0, "推廣獎金": 0, "輔具推廣獎金": 0, "慢籤成長獎金": 0,
                                "備註": df_temp["備註"] # 繼承之前的備註
                            })
                            conn.update(worksheet=PAY_SHEET, data=pd.concat([df_pay, new_r], ignore_index=True))
                            st.cache_data.clear(); st.success(f"✅ {nm} 名單已建立，備註已自動從上期繼承！"); st.rerun()
                ...
                if role == 1: # 老闆視角：連動計算
                    l_ins = get_latest_ins(df_ins, target_m)
