...
def robust_clean(df, expected_cols=None):
    ...
    mapping = {
        ...
        "狀態": "狀態", "類別": "類別", "時數": "時數", "日期": "日期", "原因": "原因", "處理方式": "處理方式", "收款帳號": "收款帳號"
    }
    new_cols = {}
    for c in df.columns:
...
def fetch_all_data():
    ...
    df_emp = robust_clean(conn.read(worksheet=EMP_SHEET, ttl=0), expected_cols=['姓名','單位','店別','身分證','電子郵件','基本薪資合計','加班時薪','補休餘額','剩餘特休時數'])
    df_pay = robust_clean(conn.read(worksheet=PAY_SHEET, ttl=0), expected_cols=['月份','姓名','加班津貼'])
    df_ins = robust_clean(conn.read(worksheet=INS_SHEET, ttl=0), expected_cols=['姓名','生效月份','加保日期','勞健保個人負擔'])
    df_acc = robust_clean(conn.read(worksheet=ACC_SHEET, ttl=0))
    df_lv = robust_clean(conn.read(worksheet=LEAVE_SHEET, ttl=0), expected_cols=['日期','姓名','狀態'])
    df_ot = robust_clean(conn.read(worksheet=OT_SHEET, ttl=0), expected_cols=['日期','姓名','狀態'])
    try: df_lock = robust_clean(conn.read(worksheet=LOCK_SHEET, ttl=0), expected_cols=['月份','狀態'])
    except: df_lock = pd.DataFrame(columns=['月份', '狀態'])
    return df_emp, df_pay, df_ins, df_acc, df_lv, df_ot, df_lock

def main():
    st.title("🚀 天康人事薪資差勤管理系統")
    ...
    if role == 5:
        ...
        tab_e = st.tabs(["💰 薪資單查詢", "📅 請假與加班申請", "🔍 差勤紀錄查詢"])
        with tab_e[0]:
            # 💡 修復 KeyError '月份'
            ...
            else: st.warning("發薪表缺少『月份』欄位")

        with tab_e[1]:
            c1, c2 = st.columns(2)
            with c1:
                st.metric("剩餘特休", f"{clean_val(e_info['剩餘特休時數'])} hr"); st.metric("補休餘額", f"{clean_val(e_info['補休餘額'])} hr")
                with st.form("l_req"):
                    st.markdown("### 🌿 請假申請")
                    ...
                    if st.form_submit_button("送出申請"):
                        conn = st.connection("gsheets", type=GSheetsConnection)
                        conn.update(worksheet=LEAVE_SHEET, data=pd.concat([df_lv, pd.DataFrame({"日期":[str(ld)],"姓名":[name],"類別":[lt],"時數":[lh],"事由":[lr],"狀態":["待審核"]})], ignore_index=True)); st.cache_data.clear(); st.success("OK")
        if st.sidebar.button("登出"): del st.session_state['auth']; st.rerun()

    else: # --- 管理端 (老闆、會計、店長) ---
        if st.sidebar.button("登出系統"): del st.session_state['auth']; st.rerun()
        
        if role == 4: # --- 會計權限 (保全 8 欄位) ---
            t_ac = st.tabs(["🏥 勞健保維護", "👤 員工名單"])
            with t_ac[0]:
                e_in = st.data_editor(df_ins, num_rows="dynamic", key="ac_ed")
                if st.button("💾 更新資料"):
                    conn = st.connection("gsheets", type=GSheetsConnection)
                    conn.update(worksheet=INS_SHEET, data=e_in); st.cache_data.clear(); st.success("OK")
            with t_ac[1]:
                df_v = df_emp[["店別", "姓名", "單位", "身分證", "電子郵件"]].copy(); df_v['店別'] = df_v['店別'].astype(str)
                st.dataframe(df_v.sort_values("店別"))

        else: # --- 老闆 與 店長 (保全鎖定與時薪保密) ---
            t_list = ["💰 薪資發薪", "📑 申請單審核中心", "👤 員工主表", "🏥 勞保記錄", "🔑 帳號管理"] if role == 1 else ["💰 薪資發薪"]
            tabs = st.tabs(t_list)
            ...
            with tabs[0]:
                ...
                if role == 1: # 老闆視角：保全應付金額
                    ...
                    for n in curr['姓名']:
                        ...
                        l_ins_list.append(v.sort_values('生效月份', ascending=False).iloc[0][['姓名', '勞健保個人負擔']] if not v.empty and '勞健保個人負擔' in v.columns else pd.Series([n, 0], index=['姓名', '勞健保個人負擔']))
                    ...
                    edited = st.data_editor(curr, key="boss_p")
                else: # 💡 店長視角 (保全時薪隱私)
                ...
                if role == 1: # 網銀 CSV 與 Email 發送 (保全標籤)
                    ...
                    with c3:
                        if st.button("📧 批量寄送 Email"):
                            for _, r in edited.iterrows():
                                ...
                                send_salary_email(r['電子郵件'], r['姓名'], target_m, r['單位'], {"應付總額": r.get('應付金額', 0)})
                            st.success("✅ 已完成")

            if role == 1:
                with tabs[1]: # 審核中心 (保全加班換錢連動)
                    st.subheader("📑 待核准申請")
                    c1, c2 = st.columns(2)
                    with c1:
                            ...
                            with st.expander(f"🌿 {row['姓名']} - {row['類別']}"):
                                if st.button("✅ 核准", key=f"la_{idx}"):
                                    ...
                                    if rule.get('deduct_balance'):
                                        ...
                                        conn = st.connection("gsheets", type=GSheetsConnection); conn.update(worksheet=EMP_SHEET, data=df_emp)
                                    df_lv.at[idx, '狀態'] = '已核准'
                                    conn = st.connection("gsheets", type=GSheetsConnection); conn.update(worksheet=LEAVE_SHEET, data=df_lv); st.cache_data.clear(); st.rerun()
                    with c2:
                        ...
                        for idx, row in p_o.iterrows():
                            with st.expander(f"⚡ {row['姓名']} - {row['時數']}hr"):
                                if st.button("✅ 同意", key=f"oa_{idx}"):
                                    conn = st.connection("gsheets", type=GSheetsConnection); df_ot.at[idx, '狀態'] = '已執行'
                                    if row['處理方式'] == '換錢':
                                        rate = clean_val(df_emp[df_emp['姓名'] == row['姓名']].iloc[0].get('加班時薪', 0))
                ...
                with tabs[2]: # 員工主表
                    ...
                    if st.button("💾 更新主表"):
                        conn = st.connection("gsheets", type=GSheetsConnection); conn.update(worksheet=EMP_SHEET, data=e_ed); st.cache_data.clear(); st.success("OK")
