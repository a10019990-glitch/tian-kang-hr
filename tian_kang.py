...
def send_salary_email(to_email, name, month, details_dict):
    ...
    <body style="font-family: Arial, sans-serif;">
        ...
        <p style="color: #7f8c8d; font-size: 12px; margin-top: 20px;">* 本信件為系統自動發送，如有疑問請洽管理部。</p>
    </body>
    </html>
    """
...
def main():
    st.title("🚀 天康藥局管理系統 (薪資全明細 Email 版)")
    ...
    if role == 5: # --- 員工專區 ---
        ...
        with t2:
            ...
            st.metric("🎯 特休餘額", f"{clean_val(ebal.get('剩餘特休時數',0))} hr"); st.metric("⚡ 補休餘額", f"{clean_val(ebal.get('補休餘額',0))} hr")
                ...
                if role == 1: # --- 老闆視角 ---
                    ...
                    with c3:
                        if st.button("📧 批量寄送 Email 全明細"):
                            count = 0
                            for _, r in pd.concat([ed_p, ed_c]).iterrows():
                                if not pd.isna(r.get('電子郵件')):
                                    # 💡 動態建立明細清單
                                    b_cols = PHARMACY_VAR if r['姓名'] in ed_p['姓名'].values else CASE_MGR_VAR
                                    d = {"基本薪資": r['基本薪資合計'], "執照津貼": r.get('執照津貼', 0), "車資補貼": r.get('車資補貼', 0)}
                                    for b in b_cols: d[b] = r[b]
                                    d.update({"本月加班時數": r['本月加班時數'], "換錢時數": r['換錢時數'], "加班津貼": r['加班津貼'], "勞健保扣款": r.get('勞健保個人負擔', 0), "實領總額": r['應付金額'], "備註": r['備註']})
                                    if send_salary_email(r['電子郵件'], r['姓名'], target_m, d): count += 1
                            st.success(f"✅ 已成功發送 {count} 封薪資明細郵件！")

                elif role == 3: # --- 店長視角 ---
                    mgr_view = curr.merge(df_emp[['姓名','單位','店別', '補休餘額', '剩餘特休時數', '加班時薪']], on='姓名', how='left')
                    ...
                    edit_cols = ["月份", "店別_y", "姓名", "現有特休", "現有補休", "本月加班時數", "換錢時數"] + PHARMACY_VAR + ["備註"]
                    for col in edit_cols:
                        if col not in mgr_view.columns: mgr_view[col] = "" if col in ["月份", "店別_y", "姓名", "備註"] else 0.0
                        if col not in ["月份", "店別_y", "姓名", "備註"]: mgr_view[col] = pd.to_numeric(mgr_view[col], errors='coerce').fillna(0.0)
                    if not mgr_view.empty:
                        ed_mgr = st.data_editor(mgr_view[edit_cols], disabled=["月份", "店別_y", "姓名", "現有特休", "現有補休"], key="mp")
