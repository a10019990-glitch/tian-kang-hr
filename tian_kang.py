...
def main():
    ...
    try:
        ...
        df_ins = robust_clean(conn.read(worksheet=INS_SHEET, ttl=300), expected_cols=['生效月份', '加保日期', '姓名', '勞健保自負額'])
    ...
    if role == 5: # 員工專區
        ...
        if role == 4:
            ...
            with t_acct[0]:
                st.subheader("🏥 勞健保明細維護")
                # 確保顯示順序：生效月份、加保日期、姓名、金額
                ins_cols = ['生效月份', '加保日期', '姓名', '勞健保自負額']
            ...
            with tabs[0]:
                ...
                if target_m != "無":
                    ...
                    if st.button("💾 同步薪資存檔"):
                        ...
                        conn.update(worksheet=PAY_SHEET, data=pd.concat([others, save_df], ignore_index=True)); st.cache_data.clear(); st.success("薪資已存檔")
            ...
            with tabs[1]: # 員工資料
                if role == 1:
                    ...
                    if st.button("💾 同步更新員工主表"): conn.update(worksheet=EMP_SHEET, data=e_emp); st.cache_data.clear()
            
            with tabs[2]: # 勞健保紀錄 (老闆版檢視)
                st.subheader("🏥 勞健保異動紀錄")
                st.dataframe(df_ins[['生效月份', '加保日期', '姓名', '勞健保自負額']].sort_values(['姓名', '生效月份'], ascending=[True, False]))
