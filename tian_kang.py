...
def main():
    ...
    if 'auth' not in st.session_state:
        # --- 登入介面 ---
        mode = st.radio("系統入口", ["管理端", "員工查詢", "新帳號註冊"], horizontal=True)
        if mode == "管理端":
    ...
    if role == 5:
        ...
        with t2:
            ...
            with st.form("apply_form"):
                st.markdown("### 📝 申請假單/加班預約")
                lt = st.selectbox("申請項目", list(LEAVE_TYPES.keys()) + ["加班預約", "補休轉現金"]); ld, lh, lr = st.date_input("日期"), st.number_input("小時", 0.5, 12.0, 1.0, 0.5), st.text_area("原因/事由")
                if st.form_submit_button("送出申請"):
                    ...
                    elif "補休轉" in lt: st.connection("gsheets", type=GSheetsConnection).update(worksheet=OT_SHEET, data=pd.concat([df_ot, pd.DataFrame({"日期":[str(ld)],"姓名":[name],"時數":[lh],"處理方式":["換錢"],"原因":["補休核現"],"狀態":["待審核"]})], ignore_index=True))
                    ...
                    st.cache_data.clear(); st.success("申請已送出！")
        
        with t3:
            st.write("🌿 我的請假紀錄"); st.dataframe(df_lv[df_lv['姓名'] == name], use_container_width=True)
            st.write("⚡ 我的加班紀錄"); st.dataframe(df_ot[df_ot['姓名'] == name], use_container_width=True)
    ...
    # --- 6. 管理端 (老闆/店長/會計) ---
            ...
            with tabs[0]:
                ...
                if role == 1: # 老闆全覽 (含分類與應付總額 ✅)
                    ...
                    if st.button("💾 老闆存檔同步"):
                        ...
                        st.connection("gsheets", type=GSheetsConnection).update(worksheet=PAY_SHEET, data=df_pay); st.success("老闆存檔完成")
                    ...
                    with c3:
                        if st.button("📧 批量寄送 Email 明細"):
                            for _, r in pd.concat([ed_p, ed_c]).iterrows():
                                send_salary_email(r.get('電子郵件'), r['姓名'], target_m, {"總額": r.get('應付金額', 0)})
                            st.success("✅ 發送完成")
                
                else: # 💡 店長視角 (開放編輯獎金與時數，物理隔離基本薪 ✅)
                    ...
                    if st.button("💾 店長存檔同步") and not is_locked:
                        for _, row in pd.concat([ed_m_p, ed_m_c]).iterrows():
                            ...
                            if not t_idx.empty:
                                df_pay.at[t_idx[0], '加班津貼'] = round(clean_val(row.get('加班時數', 0)) * rate)
                                for col in row.index:
                                    if col in (PHARMACY_VAR + CASE_MGR_VAR + ['備註']): df_pay.at[t_idx[0], col] = row[col]
        ...
        # --- 📑 申請單審核中心 (Boss 專屬：請假與加班審核 ✅) ---
        if role == 1:
            with tabs[1]:
                st.subheader("待核准申請清單")
                c1, c2 = st.columns(2)
                with c1: # 🌿 請假審核
            ...
            # --- 👤 員工資料與勞健保 (Boss/會計 100% 可視 ✅) ---
            with tabs[2]: st.data_editor(df_emp, num_rows="dynamic", key="b_m")
            with tabs[3]: st.data_editor(df_ins, num_rows="dynamic", key="b_i")
