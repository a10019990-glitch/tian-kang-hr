...
LEAVE_TYPES = {
    ...
    "流產假(年資滿半年全薪/未滿半薪)": {"pay_ratio": "Tenure_Depend", "deduct_balance": None, "desc": "5天~4週"},
...
def fetch_all_data():
    ...
    df_emp = robust_clean(conn.read(worksheet=EMP_SHEET, ttl=0), expected_cols=['姓名','單位','店別','身分證','電子郵件','基本薪資合計','執照津貼','車資補貼','加班時薪','補休餘額','剩餘特休時數'])
        ...
        if mode == "管理端登入":
            ...
            with st.form("reg"):
                ...
                if st.form_submit_button("註冊"):
                    ...
                    conn = st.connection("gsheets", type=GSheetsConnection)
                    conn.update(worksheet=ACC_SHEET, data=pd.concat([df_acc, new_u], ignore_index=True)); st.cache_data.clear(); st.success("OK")
        return

    role, shop = st.session_state.auth, st.session_state.shop

    if role == 5: # --- 員工專區 (保全查詢與新增申請) ---
        name = st.session_state.user_name.replace(" ", "")
        p_ins_rec = df_ins[df_ins['姓名'] == name]
        s_date = p_ins_rec.sort_values('加保日期').iloc[0]['加保日期'] if not p_ins_rec.empty else "N/A"
        ...
        with tab_e[0]:
            ...
            p_pay['實領總額'] = (clean_val(e_info['基本薪資合計']) + clean_val(e_info['執照津貼']) + clean_val(e_info['車資補貼']) + p_pay[b_cols].sum(axis=1)) - p_pay['勞健保個人負擔']
        ...
        with tab_e[1]:
            ...
            with c2:
                with st.form("o_req"):
                    st.markdown("### ⚡ 加班提前預約")
                    od, oh, om = st.date_input("日期"), st.number_input("小時", 0.5, 12.0, 1.0, 0.5), st.radio("處理方式", ["換錢", "換補休"])
                    if st.form_submit_button("預約加班"):
                        conn = st.connection("gsheets", type=GSheetsConnection)
                        conn.update(worksheet=OT_SHEET, data=pd.concat([df_ot, pd.DataFrame({"日期":[str(od)],"姓名":[name],"時數":[oh],"處理方式":[om],"原因":["提前申請"],"狀態":["待審核"]})], ignore_index=True)); st.cache_data.clear(); st.success("OK")
        with tab_e[2]:
                    ...
                    with c3:
                        if st.button("📧 批量寄送 Email"):
                            ...
                            for _, r in edited.iterrows():
                                ...
                                ok, _ = send_salary_email(r['電子郵件'], r['姓名'], target_m, r['單位'], {"實領總額": r['應付金額'], "備註": r['備註']})
                                if ok: sc += 1
                            st.success(f"✅ 成功寄出 {sc} 封。")

            if role == 1:
                with tabs[1]: # --- 審核中心 (保全加班自動計費) ---
                    st.subheader("📑 待核准申請")
                    c1, c2 = st.columns(2)
                    with c1:
                ...
                with tabs[2]: # 員工主表 (維護時薪、特休、補休)
                    e_ed = st.data_editor(df_emp, num_rows="dynamic", key="boss_main")
                    if st.button("💾 更新主表"):
                        conn = st.connection("gsheets", type=GSheetsConnection)
                        conn.update(worksheet=EMP_SHEET, data=e_ed); st.cache_data.clear(); st.success("OK")
                with tabs[3]: st.dataframe(df_ins.sort_values('姓名')) # 勞保紀錄
                with tabs[4]: st.dataframe(df_acc) # 帳號管理
