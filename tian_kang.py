...
def send_salary_email(to_email, name, month, total):
    ...
    html = f"<html><body><h3>👋 {name} 您好：</h3><p>您 {month} 月份的實領總額為：<b>{total}</b> 元。</p></body></html>"
...
def main():
    ...
    try:
        ...
        conn = st.connection("gsheets", type=GSheetsConnection) # 建立共用連線
    except Exception as e: st.error(f"連線失敗: {e}"); st.stop()

    if 'auth' not in st.session_state:
        # (登入頁面標籤不變)
        mode = st.radio("入口", ["管理端", "員工入口", "註冊"], horizontal=True)
        if mode == "管理端":
            ...
            e_acc = st.text_input("查詢帳號"); e_pw = st.text_input("密碼", type="password")
            if st.button("查詢登入"):
    ...
    if role == 5: # --- 員工專區 (請假系統歸位 ✅) ---
            ...
            with tabs[0]:
                ...
                if role == 1: # 🚀 老闆月份管理 (✅ 恢復歸位)
                    with st.sidebar.expander("🛠️ 月份管理系統"):
                        ...
                        if st.button("🔒 鎖定/解鎖"):
                            # 💡 修正 APIError：確保 Lock 更新邏輯穩定 ✅
                            new_lock_row = pd.DataFrame({"月份":[target_m],"狀態":["OPEN" if is_locked else "LOCKED"]})
                            others = df_lock[df_lock['月份'].astype(str) != target_m]
                            conn.update(worksheet=LOCK_SHEET, data=pd.concat([others, new_lock_row], ignore_index=True)); st.cache_data.clear(); st.rerun()
                ...
                if role == 1: # --- 老闆視角：分類與保費最新生效邏輯 ✅ ---
                    ...
                    if st.button("💾 老闆同步存檔"):
                        ...
                        conn.update(worksheet=PAY_SHEET, data=df_pay); st.success("OK")
                    ...
                    if st.button("💾 店長存檔") and not is_locked:
                ...
                with c2:
                    ...
                    for idx, row in p_o.iterrows():
                        if st.button(f"✅ 同意 {row['姓名']} - {row['處理方式']}", key=f"oa_{idx}"):
                            ...
                            df_ot.at[idx, '狀態'] = '已核准'; conn.update(worksheet=OT_SHEET, data=df_ot); conn.update(worksheet=EMP_SHEET, data=df_emp); st.cache_data.clear(); st.rerun()
        ...
        if role == 4: # 會計保全
            with tabs[0]:
                ...
                if st.button("💾 會計同步"): conn.update(worksheet=INS_SHEET, data=ed_acct); st.cache_data.clear(); st.success("OK")
