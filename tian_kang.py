...
def fetch_all_data():
    ...
    std_map = {"月份": "月份", "姓名": "姓名", "身分證": "身分證", "加保日期": "加保日期", "補休餘額": "補休餘額", "加班時薪": "加班時薪", "基本薪資合計": "基本薪資合計", "單位": "單位", "執照津貼": "執照津貼", "車資補貼": "車資補貼", "電子郵件": "電子郵件", "收款帳號": "收款帳號", "店別": "店別"}
...
def main():
    ...
    if 'auth' not in st.session_state:
        mode = st.radio("入口選擇", ["管理端", "員工查詢", "註冊"], horizontal=True)
        if mode == "管理端":
    ...
    if role == 5: # --- 員工查詢專區 ---
        ...
        with t2:
            ...
            with st.form("emp_apply"):
                ...
                if st.form_submit_button("送出"):
                    ...
                    elif "薪資" in lt: st.connection("gsheets", type=GSheetsConnection).update(worksheet=OT_SHEET, data=pd.concat([df_ot, pd.DataFrame({"日期":[str(ld)],"姓名":[name],"時數":[lh],"處理方式":["換錢"],"原因":["補休轉薪資"],"狀態":["待審核"]})], ignore_index=True))
            ...
            st.write("🔍 歷史請假紀錄"); st.dataframe(df_lv[df_lv['姓名'] == name])
            st.write("🔍 歷史加班紀錄"); st.dataframe(df_ot[df_ot['姓名'] == name])
        ...
        if "💰 薪資發薪作業" in t_titles:
            with tabs[0]:
                ...
                if role == 1: # 老闆視角：看錢 + 應付金額
                    ...
                    st.subheader("💊 藥局組發薪")
                    ed_p = st.data_editor(curr[curr['單位'] == "藥局"][['月份','店別','姓名','基本薪資合計','應付金額','電子郵件'] + PHARMACY_VAR + ['備註']], key="boss_p")
                    st.subheader("📂 個管師組發薪")
                    ed_c = st.data_editor(curr[curr['單位'] == "個管師"][['月份','店別','姓名','基本薪資合計','應付金額','電子郵件'] + CASE_MGR_VAR + ['備註']], key="boss_c")
                    
                    if st.button("💾 老闆存檔同步"):
                        for _, r in pd.concat([ed_p, ed_c]).iterrows():
                            for col in ed_p.columns:
                ...
                else: # 💡 店長視角 (隱私物理隔離 ✅ + 加班時數轉換 ✅)
                    mgr_view = curr.merge(df_emp[['姓名','單位']], on='姓名', how='left')
                    disp_l = []
                    ...
                    ed_mgr_p = st.data_editor(f_mgr[f_mgr['單位'] == "藥局"][["月份","店別","姓名"] + [c for c in PHARMACY_VAR if c != "加班津貼"] + ["加班時數","備註"]], disabled=is_locked, key="mp")
                    st.subheader("📂 個管師組 (店長限閱)")
                    ed_mgr_c = st.data_editor(f_mgr[f_mgr['單位'] == "個管師"][["月份","店別","姓名"] + [c for c in CASE_MGR_VAR if c != "加班津貼"] + ["加班時數","備註"]], disabled=is_locked, key="mc")
                    
                    if st.button("💾 店長存檔同步") and not is_locked:
                        for _, row in pd.concat([ed_mgr_p, ed_mgr_c]).iterrows():
                            rate = clean_val(df_emp[df_emp['姓名'] == row['姓名']].iloc[0].get('加班時薪', 0))
                            # 💡 修復 ValueError：改用精確 index 更新
                            target_idxs = df_pay.index[(df_pay['月份'].astype(str) == target_m) & (df_pay['姓名'] == row['姓名'])]
                            if not target_idxs.empty:
                                df_pay.at[target_idxs[0], '加班津貼'] = round(clean_val(row.get('加班時數', 0)) * rate)
                                for col in row.index:
                                    if col in ALL_VAR_COLS and col != "加班時數": df_pay.at[target_idxs[0], col] = row[col]
        ...
        if role == 4: # 會計保全
            with tabs[0]: st.data_editor(df_ins, num_rows="dynamic")
