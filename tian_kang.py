...
def send_salary_email(to_email, name, month, unit, details_dict):
    ...
    html = f"<html><body style='font-family:sans-serif;'><h2>👋 {name} 同仁：</h2><p>這是您 {month} 的薪資明細：</p><table style='border-collapse:collapse; width:100%; max-width:450px;'>{rows}</table></body></html>"
...
# --- 4. 數據讀取與快取邏輯 (解決 429 錯誤的核心) ---
@st.cache_data(ttl=60) # 快取 60 秒，減少對 Google API 的請求
def fetch_all_data():
    ...
    df_emp = robust_clean(conn.read(worksheet=EMP_SHEET, ttl=0), expected_cols=['姓名','單位','加班時薪','補休餘額','剩餘特休時數','電子郵件','基本薪資合計'])
    ...
    return df_emp, df_pay, df_ins, df_acc, df_lv, df_ot, df_lock, PHARMACY_VAR, CASE_MGR_VAR, ALL_VAR_COLS, INS_COLS

def main():
    st.title("🚀 天康薪資差勤一體化管理系統")
    
    if st.sidebar.button("🔄 刷新雲端資料 (防止流量報錯)"):
        st.cache_data.clear()
        st.rerun()

    # --- 讀取所有數據 ---
    try:
        df_emp, df_pay, df_ins, df_acc, df_lv, df_ot, df_lock, PHARMACY_VAR, CASE_MGR_VAR, ALL_VAR_COLS, INS_COLS = fetch_all_data()
    except Exception as e:
        st.error(f"連線失敗: {e}")
        st.info("💡 提示：請稍候一分鐘，或檢查 Google Sheet 分頁標題是否正確。")
        st.stop()
    ...
    if role == 5:
        ...
        with tab_e[1]:
            c1, c2 = st.columns(2)
            with c1:
                st.metric("剩餘特休時數", f"{clean_val(e_info['剩餘特休時數'])} hr")
                st.metric("補休餘額時數", f"{clean_val(e_info['補休餘額'])} hr")
                with st.form("l_req"):
                    st.markdown("### 🌿 請假申請")
            ...
            with c2:
                with st.form("o_req"):
                    ...
                    if st.form_submit_button("預約加班"):
                        conn = st.connection("gsheets", type=GSheetsConnection)
                        conn.update(worksheet=OT_SHEET, data=pd.concat([df_ot, pd.DataFrame({"日期":[str(od)],"姓名":[name],"時數":[oh],"處理方式":[om],"原因":[orsn],"狀態":["待審核"]})], ignore_index=True))
                        st.cache_data.clear(); st.success("預約成功")
        with tab_e[2]:
    ...
    # --- 管理端：老闆 (1)、會計 (4)、店長 (3) ---
        ...
        if role == 4: # 會計 (保全 8 欄位 + 排序修正)
            ...
            with tabs[0]: # 薪資作業 (保全鎖定 + 店長時薪絕對保密)
                ...
                if role == 1: # 老闆看到錢
                    ...
                    curr['應付金額'] = (curr['基本薪資合計'] + curr[ALL_VAR_COLS].sum(axis=1)) - curr['勞健保個人負擔']
            ...
            if role == 1:
                    ...
                    with c1: # 請假審核 (勞基法連動)
                        ...
                        for idx, row in p_l.iterrows():
                            with st.expander(f"🌿 {row['姓名']} - {row['類別']} ({row['時數']}hr)"):
                                p_ins_rec = df_ins[df_ins['姓名'] == row['姓名']]
                                s_date_rec = p_ins_rec.sort_values('加保日期').iloc[0]['加保日期'] if not p_ins_rec.empty else "N/A"
                                st.write(f"到職日：{s_date_rec} | 原因：{row['事由']}")
                ...
                with tabs[2]: # 員工主表 (時薪維護)
                    e_edited = st.data_editor(df_emp, num_rows="dynamic", key="boss_emp_main")
                    if st.button("💾 更新資料庫"):
                        conn = st.connection("gsheets", type=GSheetsConnection)
                        conn.update(worksheet=EMP_SHEET, data=e_edited); st.cache_data.clear(); st.success("同步完成")
