...
def main():
    ...
    if 'auth' not in st.session_state:
        # (登入介面)
        mode = st.radio("系統入口", ["管理端登入", "員工查詢入口", "新帳號註冊"], horizontal=True)
        if mode == "管理端登入":
    ...
    if role == 5:
        ...
        t1, t2, t3 = st.tabs(["💰 薪資單", "📅 申請假單/加班", "🔍 歷史紀錄"])
        with t2:
    ...
    # --- 6. 管理端 (物理隔離 ✅) ---
            ...
            with tabs[0]:
                ...
                if role == 1:
                    with st.sidebar.expander("🛠️ 月份管理系統"):
                        ...
                        # 💡 刪除功能正式復原
                        del_m = st.selectbox("選取要刪除的月份", all_m, key="del_box")
                        if st.button("🔥 執行刪除月份資料") and st.checkbox(f"我確認要刪除 {del_m} 的所有數據"):
                ...
                if role == 1: # --- 老闆全覽 (分類獎金 ✅) ---
                    ...
                    c1, c2, c3 = st.columns(3) # 網銀 CSV 標籤轉帳存入 ✅
                    ...
                    if st.button("💾 店長同步存檔") and not is_locked:
                        ...
                        conn.update(worksheet=PAY_SHEET, data=df_pay); st.cache_data.clear(); st.success("存檔完成")
        ...
        if role == 4:
            with tabs[0]:
                ed_acct = st.data_editor(df_ins, num_rows="dynamic", key="ac_v")
